    ...
    def __init__(self,head_size,max_sequence_length):
        ...
        def build_forward_with_state_op():
            ...
            def _rwkv_fwd_with_state(r, k, v, w, u, init_state=None,state_map=None):
                ...
                if init_state is not None:#shape=(B,H,D,D)
                    ...
                    if state_map is None:
                        ...
                        if n_state == 1:
                            ...
                            state_map = jnp.array([i for i in range(B)],dtype=jnp.int32)
                        else:
                            assert False,"未实现"
                    else:
                        #assert state_map is not None,"请传入一个state_map,这是一个int32类型的shape为(bz,)的数组，存放的是int_state到每一维度上的映射关系"
                        assert state_map.dtype in [jnp.int64,jnp.int32],"state_map的数值类型必须为int32"
                ...
                assert (init_state is None) == (state_map is None),"init_state与state_map必须同时传入"
            ...
            milr调用上面注册的前向过程算子，milr帮助算子创建输出张量
            """
            def _rwkv_fwd_with_state_cuda_lowering(ctx, r, k, v, w, u, s=None, s_map=None):
                r_type = ir.RankedTensorType(r.type)
                k_type = ir.RankedTensorType(k.type)
                v_type = ir.RankedTensorType(v.type)
                w_type = ir.RankedTensorType(w.type)
                ...
                if with_init_state:
                    ...
                    assert sm_type.element_type == ir.IntegerType.get_signless(32),"state_map的数据类型必须为int32"
                    #print(sm_type.shape,bz)
                    assert tuple(sm_type.shape) == (bz,),"state_map的shape 形状必须为(batch_size,)"

                    assert s_type.element_type == output_type
                    assert tuple(s_type.shape) == state_shape, "the shape of init state must be (batch_size,num_heads,head_size,head_size)"
